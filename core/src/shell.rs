...
impl<T> Bus<T> {
    ...
    pub fn push(&mut self, message: T) -> Tracking {
        let receipt = Rc::new(());
        let transaction = Tracking(Rc::downgrade(&receipt));
...
/// An iterator returned by the implementation of
/// [`IntoIterator`] for [`Bus`].
...
/// A message transaction returned by [`Shell::publish`].
#[derive(Debug, Clone)]
